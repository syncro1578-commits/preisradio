...
const BRANDS: BrandEntry[] = [
  { name: 'Samsung', slug: 'samsung', color: '#1428A0', active: true },
  { name: 'Apple',   slug: 'apple',   color: '#555555', active: true },
  { name: 'ASUS',    slug: 'asus',    color: '#00539B', active: true },
  { name: 'Sony',    slug: 'sony',    color: '#000000', active: true },
...
// ── SVG logos (fill="currentColor" → hérite la couleur au hover) ──────────────

function SamsungLogo() {
  return (
    <svg viewBox="0 0 120 32" fill="currentColor" aria-hidden="true" style={{ height: 18, width: 'auto', maxWidth: 90 }}>
...
function AppleLogo() {
  return (
    <svg viewBox="-14.558 0 270.558 315.162" fill="currentColor" aria-hidden="true" style={{ height: 26, width: 'auto' }}>
...
function AsusLogo() {
  return (
    <svg viewBox="0 0 1067.8 220.1" fill="currentColor" aria-hidden="true" style={{ height: 18, width: 'auto', maxWidth: 90 }}>
...
export default function HomeBrandShowcase() {
  return (
    <section className="mb-10" aria-labelledby="brand-showcase-heading">
      ...
      {/* Geizhals-style container — dark panel + shine animation */}
      <div className="brand-showrooms-wrap">
        <ul className="relative z-10 grid grid-cols-2 sm:grid-cols-4 gap-2">
          {BRANDS.map((brand) => {
            const logo = getBrandLogo(brand.name);
            return (
              <li key={brand.name}>
                <div
                  className={`brand-showroom-btn${brand.active ? ' active' : ''}`}
                  style={{ '--brand-color': brand.color } as React.CSSProperties}
                >
                  <Link
                    href={`/marken/${brand.slug}`}
                    className="brand-showroom-link"
